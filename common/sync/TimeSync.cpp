	...
	if (bTimingAcqu == TRUE)
	{
		...
		for (i = 0; i < iNewStIndCount; i++)
		{
				...
				(iNewStartIndexField[i] > (iCenterOfBuf - TIMING_BOUND_ABS))))
			{
...
#ifdef altreset
				if (iCorrCounter > 0) iCorrCounter--;
#else
				iCorrCounter = 0;
#endif
				iAveCorr = 0;
				...
				if (logfile != NULL)
				{
					fprintf(logfile,"Timing OK %d %d %d\n",goodct,badct,tcorr);
					fclose(logfile);
				}
				goodct++;
#endif
			...
			else
			{
				...
				if (iCorrCounter > NUM_SYM_BEFORE_RESET)
				{
					...
					if (logfile != NULL)
					{
						fprintf(logfile,"Corrected %d %d %d\n",goodct,badct,tcorr);
						fclose(logfile);
					}
					tcorr++;
					badct++;
				...
				else
				{
					...
					if (logfile != NULL)
					{
						fprintf(logfile,"yet ok %d %d %d\n",goodct,badct,tcorr);
						if (iTimeCorr >= MAX_TIMING_CORRECTIONS) 
							fprintf(logfile,"-----RESET on TOO MUCH CORR --------------\n");
						fclose(logfile);
					}
					badct++;
#endif
