...
void CFreqSyncAcq::ProcessDataInternal(CParameter& ReceiverParam)
{
	...
	if (bAquisition == TRUE)
	{
		...
		if (bFreqFound)
		{
			PostWinMessage(MS_FREQ_FOUND, 2);
			bFreqFound = FALSE;
		}
	}
	else
	{
		...
		{
			/*
			FILE * logfile = fopen("F_Log.txt","a+t");
			if (logfile != NULL)
			{
				fprintf(logfile,"Freq %d\n",(int)(ReceiverParam.rFreqOffsetAcqui * 1000000.0));
				fclose(logfile);
			}
			*/
			PostWinMessage(MS_FREQ_FOUND, 0);
			bFreqFound = TRUE;
		}
	}
}
