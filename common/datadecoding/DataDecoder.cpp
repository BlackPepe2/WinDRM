...
_BOOLEAN CDataDecoder::GetSlideShowPicture(CMOTObject& NewPic)
{
	...
	if ((DoNotProcessData == FALSE) && (eAppType == AT_MOTSLISHOW))
		bReturn = MOTSlideShow[0].GetPicture(NewPic);
...
_BOOLEAN CDataDecoder::GetSlideShowBSR(int * iNumSeg, string * bsrname, int * iHashVal)
{	
	return MOTSlideShow[0].GetPartBSR(iNumSeg,bsrname,iHashVal);
}
