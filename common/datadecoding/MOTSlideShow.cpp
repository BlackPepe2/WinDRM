	...
	if (pFiBody != NULL)
	{
		...
		if (vecMOTPicture.Size() == 0)
		{
			...
			for (i=0;i<the_startdelay;i++)
			{
				...
				vecMOTPicture[iOldNumObj].bIsLeader = TRUE;
...
void CMOTSlideShowEncoder::SetStartDelay(int delay)
{
	the_startdelay = delay;
}
int CMOTSlideShowEncoder::GetPicCnt(void)
{
	return MOTDAB.GetPicCount();
}
int CMOTSlideShowEncoder::GetPicPerc(void)
{
	return MOTDAB.GetPicSegmPerc();
}
int CMOTSlideShowEncoder::GetNoOfPic(void)
{
	return vecMOTPicture.Size();
}
...
_BOOLEAN CMOTSlideShowDecoder::GetPartBSR(int * iNumSeg, string * bsrname, int * iHash)
{
	return MOTDAB.GetActBSR(iNumSeg,bsrname,iHash);
}
