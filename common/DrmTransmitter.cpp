...
BOOL DoNotSend = TRUE;
BOOL DoSendFirst = TRUE;
BOOL DoNotSendFirst = TRUE;
...
void CDRMTransmitter::StartParameters(CParameter& Param)
{
	...
	else
	{
		...
		Param.Service[0].DataParam.iPacketLen = 127; /* TEST */
	...
	/* Service label data. Up to 16 bytes defining the label using UTF-8
	   coding */
	Param.Service[0].strLabel = getcall();
	...
	SetCarOffset(350 * S_corrfact); /* Default: "VIRTUAL_INTERMED_FREQ" */
